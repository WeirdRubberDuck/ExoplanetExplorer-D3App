...
function ParallelCoordinatesChart(chartId, data, options) {
  ...
  const dimensionsWithUncertainty = dimensions.filter(
    (d) => `${d}err1` in uncertaintyData[0] && `${d}err2` in uncertaintyData[0]
  );

  // Initialize uncertainty data structure
  dimensionsWithUncertainty.forEach((d) => {
    uncertaintySelection[d] = false;
  });
  ...
  element.render = function () {
    ...
    for (i in dimensions) {
      ...
      if (isNumber) {
        ...
        yScales[name] = d3.scalePoint().domain(domain).range([cfg.h, 0]);
      }
    }

    // Build the X scale -> it find the best position for each Y axis
    let xScale = d3.scalePoint().domain(dimensions).range([0, cfg.w]);
    ...
    if (cfg.uncertaintyData && cfg.showUncertaintyOnHover) {
      uncertaintyAreas = svg
        .selectAll('.uncertaintyArea')
        .data(data)
        .enter()
        .append('path')
      ...
      .each(function (d) {
        d3.select(this).call(
          (yScales[d].brush = d3
            .brushY()
            .extent([
              [-brushW, yScales[d].range()[1]],
              [brushW, yScales[d].range()[0]]
            ])
    ...
    function nanBrushColor(axis) {
      if (!nanBrushes[axis]) {
        return 'darkgray';
      } else if (nanBrushes[axis] === NanBrushState.Block) {
        return 'rgba(220, 0, 0)';
      } else if (nanBrushes[axis] === NanBrushState.Filter) {
        return 'rgba(0, 220, 0)';
      } else {
        return null;
      } // Shouldn't happen
    ...
    axes
      ...
      .on('click', (event, d) => {
        ...
        d3.select(event.currentTarget).attr('fill', nanBrushColor(d));
        brush();
      })
      .attr('r', 4)
      .attr('cx', (d) => {
    ...
    function clearBrushes() {
      ...
      d3.selectAll('.toClear').each(function (d) {
        d3.select(this).classed('toClear', false).call(yScales[d].brush.clear);
      });
      nanBrushes = {};
    ...
    function onBrushStart(event, axis) {
      if (!yScales[axis].brushSelectionValue) {
        ...
        d3.select(`#nanBrush-${axis}`).attr('fill', nanBrushColor(axis));
      }
    }
    ...
    function renderCheckBoxes() {
      ...
      let boxes = axes
        .append('rect')
        .filter((d) => {
          return dimensionsWithUncertainty.includes(d);
        })
    ...
    axes.call(drag);
    axes.selectAll('.legend').style('cursor', 'move');
  ...
  element.updateParallelLineColors = function (chartId, colorFunction) {
    d3.selectAll(`.parallelLine.${chartId}`).style('stroke', (d) => colorFunction(d.id));
  };
